...
def extract_drug_cline_data():
    ...
    df2 = df2[df2['IC50_PUBLISHED'].notna()]
    ic50_published = df2['IC50_PUBLISHED'].tolist()
    cline = df2['ARXSPAN_ID'].tolist()
    drugs = df2['DRUG_NAME'].tolist()
    broad_id = df2['BROAD_ID'].tolist()
    ec50 = df2['ec50'].tolist()
    dataset = df2['DATASET'].tolist()
...
def filtering_fun(drug, drugs, ic50, cellines):
...
def correlation_fun(df_data, drug, gene_names):
...
ic50, drugs, cellines = filtering_fun(drug, drugs, ic50, cellines)
...
corr_data = correlation_fun(df_data, drug, gene_names)
